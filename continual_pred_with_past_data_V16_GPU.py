...
for train_end in range(arguments.predfrom, arguments.weekmax - 4 + 1, 1):
    ...
    for it in range(arguments.iter):
        ...
        loss_value = (
            torch.sum(weights * torch.stack(losses[:5]))
        ...
        if it % 1000 == 0:
            ...
            print("ode loss: ", 
                  np.mean(losses[5].detach().cpu().numpy()),
                  np.mean(losses[6].detach().cpu().numpy()),
                  np.mean(losses[7].detach().cpu().numpy()),
                  np.mean(losses[8].detach().cpu().numpy()),
                  np.mean(losses[9].detach().cpu().numpy()),
                  np.mean(losses[10].detach().cpu().numpy()),
                  np.mean(losses[11].detach().cpu().numpy()),
                  np.mean(losses[12].detach().cpu().numpy()),
                  np.mean(losses[13].detach().cpu().numpy()))
            if loss_total[-1] < min_loss:
                min_loss = loss_total[-1]
                torch.save(
                    u.state_dict(),
                    "./checkpoints/continual_pred_with_past_data_V16_model_weights",
                )
                torch.save(
                    u1.state_dict(),
                    "./checkpoints/continual_pred_with_past_data_V16_model_weights_1",
                )

    # make predictions
    u.load_state_dict(
        torch.load("./checkpoints/continual_pred_with_past_data_V16_model_weights")
    )
